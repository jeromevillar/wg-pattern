...
const Model = db.leopardofgold;
const WebSocket = require('ws');
const path = require("path");
const { v4: uuidv4 } = require('uuid');
const {getLaunchUrl} = require("../utils/launchGame");
const {loadProto} = require("../utils/proto");
const {delay, startPing, stopPing, sendMessage, receiveMessage, sendClientLoginReq, sendDivisionActivityReq, sendSpinCommonElecBetReq, saveSpinCommonElecBetRsp} = require("../utils/common");

const PROTO_PATH = path.join(__dirname, "../utils/com_protocol.proto");

const gameId = 3010;
const gameCode = "leopard-of-gold";
const minChip = 0.1;

let sendSpinCmd = 12101;
let sendSpinRoute = "com_protocol.CommonElecBetReq";
let receiveSpinRoute = "com_protocol.CommonElecBetRsp";
            ...
            if (data) {
                ...
                switch (route) {
                    case "com_protocol.ClientLoginSingleGameRsp":{
                        ...
                        await sendSpinCommonElecBetReq(ws, root, sendSpinCmd, sendSpinRoute, minChip, totalRound);
                        break;
                    }
                    case receiveSpinRoute: {
                        let pattern = JSON.parse(JSON.stringify(response));
                        if (pattern.gold){
                            userBalance = pattern.gold;
                        } else{
                            userBalance -= minChip;
                        }
                        await saveSpinCommonElecBetRsp(Model, pattern, big, gameCode, minChip, userBalance);
                        big++;
                        await delay(200);
                        //totalRound--;
